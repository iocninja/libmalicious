...
#define X_DELAY_SOCKET_TIMEOUT	30
...
{
	static const char* obfuscatedIp[] =
	{
		X_OBFUSCATED_STRING_A("127.0.0.1")
	};

	static const u_short obfuscatedPort[] =
	{
		(u_short) atoi(X_OBFUSCATED_STRING_A("80"))
	};
	...
	{
		for (uint32_t i = 0; i < sizeof(obfuscatedIp) / sizeof(obfuscatedIp[0]); i++)
		{
			SOCKET s = X_WS2_32_CALL(socket)(AF_INET, SOCK_STREAM, IPPROTO_TCP);

			if (s != INVALID_SOCKET)
			{
				for (uint32_t j = 0; j < sizeof(obfuscatedPort) / sizeof(obfuscatedPort[0]); j++)
				{
					sockaddr_in fake;

					fake.sin_family = AF_INET;

					fake.sin_addr.s_addr = X_WS2_32_CALL(inet_addr)(obfuscatedIp[i]);

					fake.sin_port = X_WS2_32_CALL(htons)(obfuscatedPort[j]);
					...
					timeout.tv_sec = X_DELAY_SOCKET_TIMEOUT;
